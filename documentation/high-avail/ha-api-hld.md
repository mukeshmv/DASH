...
These APIs also support the mode described in [SmartSwitch HA Design - DPU-scope-DPU-driven setup](https://github.com/sonic-net/SONiC/blob/master/doc/smart-switch/high-availability/smart-switch-ha-dpu-scope-dpu-driven-setup.md), in which the DPU owns the HA state machine and drives state transitions on its own by directly communicating with its HA pair.
...
In the case of DPU driven mode, the flow table is fully managed by the DPU and is not exposed via the DASH flow APIs.
